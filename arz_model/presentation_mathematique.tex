...
\begin{frame}
    ...
    \textbf{Variables :}
    \begin{itemize}
        \item $\rho_m, \rho_c$: Densités (motos/voitures)
        \item $w_m, w_c$: Variables de momentum. $w = v + p(\rho)$
        \item $v_m, v_c$: Vitesses réelles
        \item $V_e$: Vitesse à l'équilibre
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Comprendre les Équations (1/3)}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Comprendre les Équations (2/3)}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Comprendre les Équations (3/4)}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Comprendre les Équations (4/5)}

    \textbf{Variable de moment (suite)}

    \begin{itemize}
        \item \textbf{Intuition :} Les conducteurs ne réagissent pas seulement à la densité locale, mais anticipent la densité devant eux.
        \item C'est la caractéristique principale des modèles ARZ.
    \end{itemize}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Comprendre les Équations (5/5)}

    \textbf{Qu'est-ce que la vitesse à l'équilibre $V_e(\rho)$ ?}
    \begin{itemize}
        \item C'est la vitesse que les conducteurs adopteraient dans des conditions de trafic stables.
        \item Elle dépend de la densité totale : plus il y a de monde, plus $V_e$ est faible.
        \item Une forme typique est :
              \begin{equation*}
                  V_e(\rho) = V_{max} \left(1 - \frac{\rho_{total}}{\rho_{jam}}\right)
              \end{equation*}
        \item $V_{max}$ : vitesse maximale (route vide)
        \item $\rho_{jam}$ : densité maximale (embouteillage complet)
    \end{itemize}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Étape 1 : Reconstruction Spatiale (WENO) - 1/3}
    ...
    \begin{center}
        \includegraphics[width=0.7\textwidth]{reconstruction.png}
    \end{center}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Étape 1 : Reconstruction Spatiale (WENO) - 2/3}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Étape 1 : Reconstruction Spatiale (WENO) - 3/3}
    ...
    \textbf{Avantages de WENO5 :}
    \begin{itemize}
        \item Ordre 5 de précision en zone lisse
        \item Pas d'oscillations parasites près des discontinuités
        \item Préserve la monotonicité de la solution
    \end{itemize}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Étape 2 : Le Problème de Riemann à l'Interface - 1/2}
    ...
    \begin{center}
        \includegraphics[width=0.6\textwidth]{riemann.png}
    \end{center}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Étape 2 : Le Problème de Riemann à l'Interface - 2/2}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Étape 3 : Intégration Temporelle (SSP-RK3) - 1/2}

    Maintenant que nous savons calculer la divergence des flux, nous pouvons mettre à jour la solution.
    ...
    \begin{itemize}
        ...
        \item Les oscillations peuvent apparaître et détruire la précision.
    \end{itemize}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Étape 3 : Intégration Temporelle (SSP-RK3) - 2/2}
    ...
    \begin{itemize}
        ...
        \item \textbf{SSP} signifie qu'elle préserve la stabilité que WENO a établie.
        \item C'est le partenaire idéal pour WENO.
    \end{itemize}

    \vspace{0.5cm}

    \textbf{Les 3 étapes de SSP-RK3 :}
    \begin{align*}
...
\begin{frame}
    \frametitle{Scénario de Simulation : Un Embouteillage}

    \begin{itemize}
        \item \textbf{Objectif :} Observer la formation et la propagation d'un embouteillage sur un réseau de deux segments de route (0-200m et 200-400m).
        \item \textbf{Cause :} Nous avons simulé une forte densité de véhicules à la fin du premier segment (à 200m), comme un feu rouge ou un rétrécissement.
        \item \textbf{Observation :} Nous allons visualiser la densité des motos et la vitesse des voitures pour voir comment le système réagit.
    \end{itemize}

\end{frame}

\begin{frame}
    \frametitle{Résultats : Segment 1 (0-200m)}

    \textbf{Densité des Motos ($\rho_m$) et Vitesse des Voitures ($v_c$)}
    
    \vspace{0.5cm}
    \textbf{Analyse :}
    \begin{itemize}
        \item Une \textbf{onde de choc} (embouteillage) se forme à 200m et se propage vers l'arrière (vers x=0).
        \item Pour la densité des motos : La densité augmente fortement (zone de haute densité en jaune/vert dans le graphique).
        \item Pour la vitesse des voitures : La vitesse chute drastiquement dans la zone de forte densité (couleur violet foncé), passant de 50 km/h à presque 0 km/h.
        \item On observe clairement la relation inverse entre densité et vitesse : plus il y a de véhicules, plus la vitesse diminue.
    \end{itemize}

\end{frame}

\begin{frame}
    \frametitle{Résultats : Segment 2 (200-400m)}
    ...
    \textbf{Analyse :}
    \begin{itemize}
        \item Le segment en aval de l'embouteillage reste fluide.
        \item Pour la densité des motos : La densité reste très faible (violet sur le graphique), car les véhicules sont bloqués sur le segment précédent et n'arrivent qu'au compte-gouttes.
        \item Pour la vitesse des voitures : La vitesse est élevée (vert/jaune sur le graphique), atteignant jusqu'à 50 km/h, ce qui est cohérent avec un trafic parfaitement fluide.
        \item Cette différence entre les deux segments illustre parfaitement la propagation des ondes de congestion et la transmission des conditions de trafic d'un segment à l'autre.
    \end{itemize}

\end{frame}

\begin{frame}
    \frametitle{Conclusion de la Simulation}

    \textbf{La simulation est un succès !}

    \begin{itemize}
        \item \textbf{Cohérence physique :} Le modèle a correctement reproduit la dynamique d'un embouteillage. Une haute densité correspond bien à une faible vitesse.
        \item \textbf{Propagation d'onde :} On observe clairement la propagation de l'onde de choc vers l'amont, ce qui est un phénomène bien connu en trafic routier.
        \item \textbf{Robustesse du code :} La chaîne numérique, de la discrétisation à la résolution sur GPU, a fonctionné de manière stable et a produit des résultats physiquement réalistes.
        \item \textbf{Validation du modèle :} Ces résultats valident que notre implémentation du modèle ARZ est capable de capturer des phénomènes complexes et pertinents pour l'étude du trafic.
    \end{itemize}
