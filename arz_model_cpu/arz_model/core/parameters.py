...
class ModelParameters:
    ...
    def load_from_pydantic(self, config: 'NetworkSimulationConfig'):
        ...
        self.rho_jam = phys.rho_jam / 1000.0
        ...
        # The IC and BC configs are complex objects. The legacy `initial_conditions`
        # and `boundary_conditions` were simple dicts. We will let the new
        # IC/BC builders handle the Pydantic objects directly, so we don't
        # need to translate them here. We just store them.
        self.initial_conditions = config.ic.dict()
        self.boundary_conditions = config.boundary_conditions.dict()
