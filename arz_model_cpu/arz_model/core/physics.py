...
def calculate_equilibrium_speed(rho_m: np.ndarray, rho_c: np.ndarray, R_local: np.ndarray, params, 
    ...
    Args:
        ...
        params: ModelParameters or SimulationConfig object.
    ...
    """
    # --- Polymorphic Parameter Extraction ---
    # This block allows the function to work with both the legacy ModelParameters
    # class and the new Pydantic NetworkSimulationConfig object.
    if hasattr(params, 'physics') and params.physics is not None:
        phys_params = params.physics
        rho_jam = phys_params.rho_jam
        V_creeping = phys_params.v_creeping_kmh * KMH_TO_MS
        Vmax_m = {1: phys_params.v_max_m_kmh * KMH_TO_MS}
        Vmax_c = {1: phys_params.v_max_c_kmh * KMH_TO_MS}
    else: # Legacy ModelParameters
        phys_params = params
        rho_jam = phys_params.rho_jam
        V_creeping = phys_params.V_creeping
        Vmax_m = phys_params.Vmax_m
        Vmax_c = phys_params.Vmax_c
    # --- End Polymorphic Extraction ---
    ...
    # If R_local is not provided, assume a default road type (e.g., 1)
    if R_local is None:
        R_local = 1  # Default road type

    # Determine local max speeds based on road type R_local, with fallback to default
    Vmax_m_local = Vmax_m.get(R_local, Vmax_m.get(1))
    Vmax_c_local = Vmax_c.get(R_local, Vmax_c.get(1))

    # Calculate reduction factor g, ensuring it's between 0 and 1
    g = np.maximum(0.0, 1.0 - rho_total / rho_jam)

    # Add creeping speed
    Ve_m = Vmax_m_local * g + V_creeping
    Ve_c = Vmax_c_local * g + V_creeping
...
def calculate_relaxation_time_gpu(rho_m_i: float, rho_c_i: float,
    ...
    return tau_m, tau_c
def calculate_relaxation_time(rho_m: np.ndarray, rho_c: np.ndarray, params) -> tuple[float, float]:
    """
    ...
    if hasattr(params, 'physics') and params.physics is not None:
        phys_params = params.physics
        tau_m = phys_params.tau_m
        tau_c = phys_params.tau_c
    else: # Legacy ModelParameters
        phys_params = params
        tau_m = phys_params.tau_m
        tau_c = phys_params.tau_c
...
@njit
def calculate_eigenvalues(rho_m: np.ndarray, v_m: np.ndarray, rho_c: np.ndarray, v_c: np.ndarray, 
                          alpha: float, rho_jam: float, epsilon: float, 
                          k_m: float, gamma_m: float, k_c: float, gamma_c: float) -> list:
    """
    Calculates the four eigenvalues of the ARZ model Jacobian matrix.

    Args:
        rho_m, v_m, rho_c, v_c: State variables (densities in veh/m, velocities in m/s).
        alpha, rho_jam, epsilon, k_m, gamma_m, k_c, gamma_c: Scalar physics parameters.

    Returns:
        A list [lambda1, lambda2, lambda3, lambda4] of eigenvalues.
    """
    # Ensure densities are non-negative for calculations
    rho_m = np.maximum(rho_m, epsilon)
    rho_c = np.maximum(rho_c, epsilon)

    rho_eff_m = rho_m + alpha * rho_c
    rho_total = rho_m + rho_c

    # Partial derivatives of pressure
    term1_m = (gamma_m * k_m / rho_jam) * np.power(np.maximum(0, rho_eff_m / rho_jam), gamma_m - 1)
    dp_mdrho_m = np.where(rho_eff_m > epsilon, term1_m, 0.0)

    term1_c = (alpha * gamma_m * k_m / rho_jam) * np.power(np.maximum(0, rho_eff_m / rho_jam), gamma_m - 1)
    dp_mdrho_c = np.where(rho_eff_m > epsilon, term1_c, 0.0)

    term2 = (gamma_c * k_c / rho_jam) * np.power(np.maximum(0, rho_total / rho_jam), gamma_c - 1)
    dp_cdrho_m = np.where(rho_total > epsilon, term2, 0.0)
    dp_cdrho_c = np.where(rho_total > epsilon, term2, 0.0)

    lambda1 = v_m
    lambda2 = v_m - rho_m * dp_mdrho_m
    lambda3 = v_c
    lambda4 = v_c - rho_c * dp_cdrho_c
