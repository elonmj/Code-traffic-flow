...
class RLNetworkConfigBuilder:
    ...
    def simple_corridor(
        ...
        global_params = {
            ...
            'red_light_factor': 0.01,  # ✅ STRENGTHENED: Very strong blocking (1% flow) during RED signal to force congestion
        ...
        boundary_conditions = BoundaryConditionsConfig(
            left=InflowBC(
                state=BCState(
                    rho_m=0.16,    # INCREASED: 80 veh/km inflow (higher urban demand to force congestion)
                    w_m=11.556,    # w_m = v_m + p_m (ARZ Lagrangian variable)
                                   # With rho_m=0.16, rho_c=0.08: p_m ≈ 2.67
                                   # v_m = 8.89 m/s → w_m = 8.89 + 2.67 = 11.56
                    rho_c=0.08,    # INCREASED: 40 veh/km inflow
                    w_c=16.823     # w_c = v_c + p_c
                                   # With rho_c=0.08: p_c ≈ 2.93
                                   # v_c = 13.89 m/s → w_c = 13.89 + 2.93 = 16.82
                )
            ),
            right=OutflowBC(
                state=BCState(
        ...
        config = NetworkSimulationConfig(
            ...
            t_final=t_final,      # 1 hour simulation
