...
class Node:
    ...
    Academic Note:
        ...
        where nodes are connection points with conservation laws:
            sum(flux_in) = sum(flux_out)  (mass conservation at junctions)
    """
    ...
    def __init__(
        ...
        """
        if not incoming_segments and intersection:
            raise ValueError(f"Node {node_id} must have at least one incoming segment if an intersection is provided.")
        if not outgoing_segments and intersection:
            raise ValueError(f"Node {node_id} must have at least one outgoing segment if an intersection is provided.")
            
        self.node_id = node_id
        self.position = position
        self.intersection = intersection
        self.incoming_segments = incoming_segments or []
        self.outgoing_segments = outgoing_segments or []
